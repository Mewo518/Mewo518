...
#include <unordered_set>			//用于unordered_set<string> 
#include <vector>
/*Protobuf是用于序列化结构化数据的协议，它的数据格式更加紧凑、快速、灵活。
.pb.h是Protocol Buffers（Protobuf）的C++语言生成的源文件，
用于实现与.proto文件定义的数据结构的序列化和反序列化
通过定义.proto文件来描述数据结构，使用Protobuf可以方便地进行数据序列化、反序列化、传输等操作。
.pb.h该文件包含了由.proto文件自动生成的C++代码，可以直接在应用程序中使用。*/
#include "tensorflow/core/framework/attr_value.pb.h"
#include "tensorflow/core/framework/node_def.pb.h"	//NodeDef类 
...
//总结上述头文件： .proto文件定义数据类型，自动生成.pb.h 
namespace tensorflow {
	//namespace是一个用于定义一组相关函数、变量或类的命名空间。
	//使用namespace可以避免命名冲突和代码混乱，使代码更易于维护和管理
	//使用方法：tensorflow：：变量或者方法 
namespace grappler {
...
//todo 要进行的操作，将此列表替换为成本模型

//unordered_set 无序集合，无序但保证字符串唯一 	unordered_set<string>存储字符串的集合 
std::unordered_set<string> GetCheapToRecomputeOps() {        //GetCheapToRecomputeOps函数 
  std::unordered_set<string> cheap_ops = {"Add",			//cheap_ops 变量 
//::作用域分解运算符，using namespace 名字，用于释放 命名空间中的变量名 
                                          "AddN",
                                          "BiasAdd",
                                          "Cast",
                                          "Fill",
...
// Find recomputable ops which feed into target nodes.feed into target nodes.反馈到目标节点 
//此函数查找候选重计算节点，返回candidate_recompute_nodes
...
{//上面四个是函数的输入参数 NodeMap&类似于int*  加const是因为保证数据不变性提高安全性，如使代码中的 NodeDef里面都是一些常量1，2，3，为了让其不变，不被其他变量给改了
  std::unordered_set<const NodeDef*> candidate_recompute_nodes;
  for (const auto& node : graph->node()) {	//节点是传入图中的节点
    if (!is_candidate(node)) {	//is_candidate是传入的函数，对候选的节点一律不看 就继续for
      continue;
    }
    bool has_target_output = false;//node_map是传入的图，到底传入什么不清楚
    for (const NodeDef* output : node_map.GetOutputs(node.name())) {
      // It only makes sense to recompute this if it feeds into a target	 只有在它为某个目标节点提供输入的情况下，才需要重新计算
      // node. We expand this to dependencies in GetOpGroupsToRecompute.
      if (is_target(*output)) {	//is_target是传入的函数，输出output是目标节点就has_target_output改为true继续for
    ...
    for (const string& input_name : node.input()) {//node为正在遍历图的节点
      // Don't recompute nodes which depend on target nodes.不要重计算依靠目标节点的节点
    ...
    candidate_recompute_nodes.insert(&node);	//符合两个要求就加入
  }
